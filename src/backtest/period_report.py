...
def compute_period_rollups(
    ...
    if equity_curve.empty:
        result.update(end_equity=start_equity, net_pnl=0.0, trading_days=0)
        return result
    ...
    result["net_pnl"] = balance - start_equity
...
def print_period_report(
    ...
    print(f"Net PnL:    {_usd(net)} ({net / start:+.1%} on starting balance)")
    ...
    for day, row in rollups["daily"].items():
        rows.append([day, _usd(row["start_equity"]), _usd(row["pnl"]), _usd(row["end_equity"])])
    if rows:
        print(tabulate(rows, headers=["Date", "Day open", "Day PnL", "Day close"], tablefmt="simple"))
    else:
